...
const io = require('socket.io')(server, {
    cors:
    {
        ...
        methods: ['GET', 'POST'],
    } // this is the method which we are going to use in socket io.
...
io.on('connection', socket => {
    ...
    socket.on('user-login', userID => {
        if (!onlineUsers.includes(userID)) {
            onlineUsers.push(userID); // if user is not already in the onlineUsers array then push it.
        }
        socket.emit('online-users', onlineUsers); // emit the onlineUsers array to all the connected clients.
    })
})
